...
const clamp = (v: number, min: number = 0, max: number = 100): number => Math.min(Math.max(v, min), max);
const round = (v: number, precision: number = 3): number => parseFloat(v.toFixed(precision));
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const tiltEngine = useMemo(() => {
    ...
    const setVarsFromXY = (x: number, y: number) => {
      ...
      for (const [k, v] of Object.entries(properties)) {
        (wrap.style as any).setProperty(k, v);
      }
    };
  ...
  const getOffsets = (evt: MouseEvent, el: HTMLElement): { x: number; y: number } => {
  ...
  const handlePointerMove = useCallback(
    (event: PointerEvent) => {
      ...
      const { x, y } = getOffsets(event as any, shell);
  ...
  const handleDeviceOrientation = useCallback(
    (event: DeviceOrientationEvent) => {
      ...
      const x = clamp(centerX + (gamma || 0) * mobileTiltSensitivity, 0, shell.clientWidth);
      const y = clamp(
